    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          //       //     Navigator.push(
          //       //       context,
          //       //       MaterialPageRoute(builder: (context) => MyPostPage(post: post)),
          //       //     );
          //       //   },
          //       // ),
          //
...
class PostDetailsPage extends StatelessWidget {
  ...
  Future<void> deletePost(accessToken, BuildContext context) async {
    try {
      if (post.title == 'Ride Offer') {
        final response = await http.delete(
          Uri.parse('http://192.168.1.77:8000/api/PostRideOffer/${post.id}/destroy'),
          headers: {
        ...
        if (response.statusCode == 200) {
          ...
          print('Ride Offer post deleted successfully');
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
          ...
          // Request failed, handle the error
          var responseData = jsonDecode(response.body);
          print('Error deleting Ride Offer post: $responseData');
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text('Error'),
                content: Text('Error deleting Ride Offer post: $responseData'),
                actions: [
                  TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
        ...
        final response = await http.delete(
          Uri.parse('http://192.168.1.77:8000/api/PostRideRequest/${post.id}/destroy'),
          headers: {
            'Authorization': 'Bearer $accessToken',
            'Content-Type': 'application/json',
          },
        ...
        if (response.statusCode == 200) {
          // Request successful, handle the response if needed
          var responseData = jsonDecode(response.body);
          print('Ride Request post deleted successfully');
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
          ...
          // Request failed, handle the error
          var responseData = jsonDecode(response.body);
          print('Error deleting Ride Request post: $responseData');
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text('Error'),
                content: Text('Error deleting Ride Request post: $responseData'),
                actions: [
                  TextButton(
                    onPressed: () {
                      Navigator.of(context).pop();
            ...
            child: Align(
              ...
              child: Container(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    deletePost(accessToken, context);
